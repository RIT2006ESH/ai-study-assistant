...
async def generate_summary(
    ...
    try:
        ...
        if len(text_to_summarize) > max_length:
            print(f"Text too long ({len(text_to_summarize)} chars), chunking...")
            ...
            print(f"Combined chunk summaries, final length: {len(text_to_summarize)} chars")
        ...
        return SummaryResponse(
            ...
            original_length=document.text_length if document.text_length else len(document.extracted_text)
        )
...
async def analyze_image(
    ...
    """
    Analyze an image containing problems, diagrams, or notes
    """
    try:
        # Read image file
        contents = await file.read()
        ...
        # Note: For actual image analysis, you'd use GPT-4 Vision or similar
        # This is a placeholder that explains what would happen
        prompt = """Based on the image uploaded, provide analysis in this format:

If the image contains:
- Math problems: Provide step-by-step solutions
- Diagrams: Explain the concepts illustrated
- Notes or text: Summarize key points
- Charts/Graphs: Interpret the data

Be educational and thorough in your explanation."""

        messages = [
            {"role": "system", "content": "You are a helpful study assistant analyzing images."},
            {"role": "user", "content": prompt}
        ]
        ...
        return {
            ...
            "analysis": analysis,
            "message": "Image analyzed successfully (Note: Using placeholder - integrate GPT-4 Vision for real image analysis)"
        }
...
async def transcribe_audio(
    ...
    try:
        ...
        # Validate file type
        valid_audio_types = ['audio/mpeg', 'audio/mp3', 'audio/wav', 'audio/m4a', 'audio/webm']
...
Be thorough and educational."""
        ...
        print(f"Solution generated successfully, length: {len(solution)} chars")
